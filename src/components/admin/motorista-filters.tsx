...
interface MotoristaFiltersProps {
  search: string;
...
export default function MotoristaFilters({
  search,
  setSearch,
  statusFilter,
  setStatusFilter,
  ...
  return (
    <div className="bg-white p-6 rounded-[10px] border border-slate-100 shadow-sm space-y-4">
          ...
          <div className="flex items-center gap-1 p-1 bg-slate-50 rounded-[10px] border border-slate-100">
            {(["todos", "ativos", "inativos"] as const).map((f) => (
              <button
                ...
                className={`px-4 py-2 rounded-[10px] text-[10px] font-semibold uppercase tracking-wider transition-all ${
                  statusFilter === f
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-slate-50">
        {/* Filtro de Categoria da Viatura */}
