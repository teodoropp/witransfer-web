...
export default function ViaturasPage() {
  ...
  const toggleStatus = async (id: string, currentStatus: boolean) => {
    try {
      ...
      if (error) throw error;
      setViaturas(prev => prev.map(v => v.id === id ? { ...v, ativo: !currentStatus } : v));
    } catch (err) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-black text-slate-800 tracking-tight">Gestão de Frota</h2>
          <p className="text-slate-500 font-medium mt-1">Gira as viaturas disponíveis para reservas e parceiros.</p>
        </div>
        <Link 
      ...
      {loading ? (
            ...
            >
              ...
              <div className="relative h-48 w-full bg-slate-50 overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex gap-2">
                  <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm ${viatura.ativo ? 'bg-emerald-500 text-white' : 'bg-rose-500 text-white'}`}>
              ...
              <div className="p-6 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-lg font-black text-slate-800 leading-tight mb-1">{viatura.marca} {viatura.modelo}</h3>
                  ...
                  <div className="text-right">
                    <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Preço Base</span>
                    <span className="text-lg font-black text-primary leading-none">
                      {viatura.preco_base?.toLocaleString('pt-AO')} <small className="text-[10px]">Kz</small>
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 pt-4 border-t border-slate-50 mt-auto">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs font-bold text-slate-600">{viatura.lugares} Lugares</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <Briefcase size={14} className="text-slate-400" />
                    <span className="text-xs font-bold text-slate-600">{viatura.malas} Malas</span>
                  </div>
                  <div className="ml-auto">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Parceiro</span>
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[40px] border-2 border-dashed border-slate-100">
          ...
          <h3 className="text-xl font-bold text-slate-800">Nenhuma viatura encontrada</h3>
          <p className="text-slate-500 mt-1 mb-8">Tente ajustar os seus filtros ou adicionar uma nova viatura.</p>
          <Link 
            href="/admin/viaturas/nova"
